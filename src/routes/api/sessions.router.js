...
import {generateToken, authToken} from "../middlewares/jwt.js"

const PRIVATE_KEY = "ksdhjflkjshdflkjsdhflksjdhflksjdfh"
...
sessionsRouter.post("/register",passport.authenticate("register",{failureRedirect:"/api/sessions/failRegister"}),(req,res)=>{
    ...
    res.send({status: "success", access_token, message: "User registered"})
})
...
sessionsRouter.post("/login",passport.authenticate("login",{failureRedirect:"/api/sessions/failLogin"}),(req,res)=>{
    ...
    res.send({status: "success", access_token, payload: req.user})
})
